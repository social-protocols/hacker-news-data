...
query = "
    with cumulatives as (
        SELECT
            ...
            , sum(upvotesByTick.upvotes * upvoteShare) AS expectedUpvotes
            , SUM(upvotesByTick.upvotes * upvoteShare) OVER (PARTITION BY id ORDER BY tick ROWS UNBOUNDED PRECEDING) cumulativeExpectedUpvotes
        FROM 
            (SELECT * FROM random_sample_100_stories LIMIT 15   )
            JOIN dataset USING (id)
            JOIN upvotesByTick USING (tick)
            JOIN averageUpvotesByTopRank ON rank = topRank
